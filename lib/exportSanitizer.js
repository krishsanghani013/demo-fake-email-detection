...
export function sanitizeForExport(snapshot) {
  ...
  const rawEvList = Array.isArray(snapshot.risk?.evidence) && snapshot.risk.evidence.length > 0
    ? snapshot.risk.evidence
    : Array.isArray(snapshot.evidence) && snapshot.evidence.length > 0
    ? snapshot.evidence
    : Array.isArray(snapshot.risk?.breakdown)
    ? snapshot.risk.breakdown
    : [];

  const sanitizedEvidence = rawEvList.map((e, idx) => ({
    id: e.id || `EV-${String(idx + 1).padStart(3, "0")}`,
    category: e.category || "ai",
    type: e.type || "forensic_finding",
    source: e.source || "analyzer",
    evidence: e.evidence || "",
    artifact: e.artifact || null,
    severity: e.severity || "medium",
    confidence: typeof e.confidence === "number" ? e.confidence : 0.9,
    riskContribution: e.riskContribution ?? e.contribution ?? 0,
    contribution: e.riskContribution ?? e.contribution ?? 0,
    explanation: e.explanation || e.finding || "",
    finding: e.explanation || e.finding || "",
  }));

  const risk = {
    score: snapshot.risk?.score || 0,
    level: snapshot.risk?.level || "medium",
    verdict: snapshot.risk?.verdict || "LOW / LEGITIMATE",
