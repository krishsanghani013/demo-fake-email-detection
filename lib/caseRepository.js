 ...
 */
export async function createInvestigationCase(investigation, userId = null) {
  if (!db) {
  ...
  console.log(`[DB] Creating case: ${caseNumber} for user: ${userId || "none"}`);
 ...
 */
export async function getCaseById(caseIdentifier, userId = null) {
  if (!db || !caseIdentifier) return null;

  const idOrNumberClause = {
    OR: [{ caseNumber: caseIdentifier }, { id: caseIdentifier }],
  };

  const where = userId
    ? { AND: [idOrNumberClause, { userId }] }
    : idOrNumberClause;
 ...
 * When userId is provided, scopes statistics exclusively to that user.
 */
export async function getDashboardStats(userId = null) {
  ...
  const [totalCases, maliciousCount, reviewCount, benignCount, aggregate] =
    await Promise.all([
      ...
      db.case.count({ where: { ...baseWhere, classification: "FRAUDULENT" } }),
      db.case.count({ where: { ...baseWhere, classification: "SUSPICIOUS" } }),
      db.case.count({ where: { ...baseWhere, classification: "LEGITIMATE" } }),
      db.case.aggregate({
        where: baseWhere,
        _avg: {
          riskScore: true,
 ...
 * When userId is provided, enforces that the case belongs to the user.
 */
export async function deleteCase(caseIdentifier, userId = null) {
 ...
 * When userId is provided, enforces that the case belongs to the user.
 */
export async function updateCaseStatus(caseIdentifier, status, userId = null) {
  if (!db || !caseIdentifier) return null;
