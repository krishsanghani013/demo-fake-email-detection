...
export function createInvestigationSnapshot(result) {
  ...
  const rawEvidence = Array.isArray(result.evidence) && result.evidence.length > 0
    ? result.evidence
  ...
  const risk = {
    ...
    verdict: result.verdict || (result.classification === "fraudulent" ? "CRITICAL / PHISHING" : result.classification === "suspicious" ? "HIGH / SUSPICIOUS" : "LOW / LEGITIMATE"),
    classification: result.classification || "suspicious",
    confidence: result.confidence || 0,
    categoryScores: result.categoryScores || {},
    evidence: rawEvidence,
    breakdown: rawEvidence,
  };
