...
export async function syncCasesWithDatabase() {
  ...
  try {
    ...
    if (!res.ok) return localCases;

    const json = await res.json();
    if (json?.success && json?.source === "database" && Array.isArray(json.data)) {
      const dbCases = json.data;
      if (dbCases.length === 0) return localCases;

      // Merge: dbCases take precedence, plus local demo cases not in db
      const dbMap = new Map(dbCases.map((c) => [c.caseId, c]));

      const combined = [...dbCases];
      for (const lc of localCases) {
        if (!dbMap.has(lc.caseId)) {
          combined.push(lc);
        }
      }
      ...
      // Save merged list
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(combined.slice(0, MAX_STORED_CASES)));
      } catch (e) {
        // quota limit guard
      }
